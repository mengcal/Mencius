"""
米娅「工作平台·小全车间」异步派活服务 —— 装配层（create_app）
=====================================================================
爸爸拍板定稿（2026-08-27）+ 优化：
- 静默生产车间，无前端。米娅派小腿儿（临时子分身）带指令对接。
- 异步派活：任务提交立即返回 run_id（不卡），后台牛马执行。
...
（拆分后 office.py 为装配壳：`from office.app import app`，见 REFACTOR_NOTES.md）
...
from .routers import external, gates, lark, misc, providers, rag, tasks, token_admin
...
# R79③（小蝶 P0/Cora·NOVA P1）：/rag/ingest·/rag/rebuild 无鉴权=提示注入持久化面，进守护名单；
# R10.2（Eve P2 反向假边界）：/rag/query【移出】管理员名单——提示词三扇门承诺沙箱可达，
...
def create_app() -> FastAPI:
    ...
    """
    _app = FastAPI(title="小全车间", version="1.0")  # 原 :48
    ...
    _app.include_router(lark.router)  # r41 飞书桥（外发端点已在 _TOKEN_GUARDED）
    _app.include_router(tasks.router)
    _app.include_router(misc.router)
    _app.include_router(external.router)  # r58 对外派活一期（登记/回调，token 门+子钥）
    return _app
