...
【模型不硬编码（爸爸铁律 2026-08-29）】识图模型完全由配置决定（唯一真源=设置页），优先级：
  1. 设置页 vision 节（settings.json: vision.provider / vision.model）
  2. 设置页 agents 节的 visual 牛马（R65：不再读已废除的 agents_config.json）
...
def _vision_model():
    ...
    try:
        ...
        pass
    # 2) 设置页 agents 节的 visual 牛马（Sub-agents 页可改，R65 起经 settings_mgr 读）
    try:
    ...
    raise RuntimeError(
        ...
        "指定服务商和模型。识别什么模型用哪个，由爸爸决定。"
    )


def _read_b64(image_path: str) -> str:
    """读图片文件 → base64。
    R79 补（hy4 复检抓到）：旧版裸 open 任意路径=本端点成了远程文件读取器——
    workplatform 自身挂着 /data/secrets 与源码卷，沙箱内一条 POST /vision
