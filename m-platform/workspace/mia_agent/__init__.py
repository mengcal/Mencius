r"""
米娅「工作平台·小全车间」多模型引擎（可配置版）—— 拆分包
=====================================================================
（本 docstring 承接自原 D:\m\workspace\agent_multimodel.py 的模块 docstring，原 L1-9。
 拆分对照表见同目录 REFACTOR_NOTES.md；入口 agent 在 mia_agent/graph.py。）
爸爸拍板定稿（2026-08-27）+ R65 清硬编码（2026-09-01）：
  ① 牛马矩阵唯一真源 = 设置页 settings.agents（经 settings_mgr.load_agents_config 取，本模块不再自建 loader）
  ② 服务商 = 设置页 external 节（按名字查，名字对不上宁报错不越权）
  ③ 成本分层：平时便宜小模型，大活才临时升配（米娅 set_model 升降配，省钱第一）
