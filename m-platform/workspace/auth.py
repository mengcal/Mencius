# -*- coding: utf-8 -*-
"""auth.py — langgraph 原生 API 鉴权（R80 Eve b 方案·纵深防御）
...
- @auth.on 全放行：能过 authenticate 的（真密钥或 office 豁免）即有权（米娅/沙箱过不来）。
"""
...
# 双保险+防未来架构变化；R80 续按 Eve P1 补精确路径变体，避免"尾斜杠一字之差"坑下一轮）
_OFFICE_ALLOW = (
...
async def authenticate(request: Request) -> dict:
    ...
    # （派活/汇报/起标题）需要一个"进程身份"通道。R10.8（Eve P0-2/NOVA B/千问 P1-1）：
    # 网络位置豁免（源 127.0.0.1+端口 8000）被打穿——容器内米娅的 shell 同样是回环源，
    # 一条 curl 即免 token 读全部对话。现改【进程身份】为主判据：X-Internal-Key 必须
    # 等于同进程 internal_key.INTERNAL_KEY（启动时随机生成，只活在该进程内存里，
    # 米娅的 shell 进程 env 继承拿不到）——网络回环保留为第二道条件。
    import hmac as _hk
    ...
    # R10.8d（爸爸实测 401 根因）：凭证优先级修正——X-Api-Key 在本部署里是 LangSmith 的 key
