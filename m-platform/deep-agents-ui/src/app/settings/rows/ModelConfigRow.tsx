 ...
 * 原 page.tsx L346-392 原样迁出。W1：字号归四档 token，卡片壳统一 cardClass。
 */

import { useState } from 'react';
import { inputC, autoGrow, cardClass } from '../ui';
    ...
    <div className={cardClass}>
      <button className="flex w-full items-center justify-between px-3 py-2" onClick={() => setOpen(!open)}>
        <div className="flex items-center gap-2">
          ...
          {hasOverride && (
            <span className="text-xxs text-blue-500 border border-blue-500/40 rounded px-1">已单独设置</span>
          )}
        </div>
        <span className="text-xxs text-muted-foreground">{open ? '收起 ▲' : '单独设置 ▼'}</span>
      </button>
      {open && (
        <div className="border-t border-border p-3">
          <label className="mb-1 block text-sm text-muted-foreground">系统提示词（留空用全局）</label>
          <textarea
            className={inputC}
          ...
          <div className="mt-2 flex gap-3">
            ...
            <div className="flex-1"><label className="mb-1 block text-sm text-muted-foreground">max_tokens</label>
          ...
          <div className="mt-1 text-xxs text-muted-foreground">服务商：{provider} · 保存到 settings.json 的 model_overrides 节</div>
        </div>
      )}
    </div>
  );
