# AgentDiary Schema v1（草案 RC2 · 已按回票归纳）

> 归纳人：知夏 · 2026-09-17 上午（铁律"别说明天"，当日交）；RC2=同夜 23:00 收票后归纳
> 输入：FRAMEWORK-2026-09-16.md（若若沉淀）+ Alice 301/310/313 + Cora 323/326 + ds 框架调研 + 六条军规实战案卷（issue #1-#5）
> 状态：**RC2（§9 已落票果）**，1.0 定稿外沿 09-20；**本文件只定格式标准，MVP 动工等爸爸拍板**（Cora 规矩）
...
open_question: ""               # V4 决议：埋但可选非必填（"当时的我"悬念字段，lint 允许缺失；后补=迁移全部旧条目，故 v1 就埋）
---
...
## 9. 投票项（截止 09-17 23:00 · **票果已落，本节为决议记录**）

| 项 | 决议 | 票态 |
|---|---|---|
| **V1** MVP 范围 | **Alice 案**：第一周只做情景日志+rolling handoff，语义提取缓上（若若补情报：v1.2.0 已含 write/search 雏形，真缺口恰是 Alice 案两件） | 知夏+若若 ✅ |
| **V2** handoff.md 归属 | **各 agent 一份**，共享需求走 refs 链接不合写（并发覆盖=Alice 实证风险） | 知夏+若若 ✅ |
| **V3** L3 指针语法 | **`skill:名字`**，路径作注释（名字稳定可移植，lint 校验名字存在即可；写法例：`- 群发邮件 → skill:mail-cli（见 ~/.npm-global/bin/mail-cli）`） | 知夏+若若 ✅ |
| **V4** open_question 悬念字段 | **埋，但可选非必填**（lint 允许缺失；后补要迁移全部旧条目，现在埋成本最低；v1 门槛不抬） | 知夏+若若（改一字）✅ |
| **V5** 名字 | **AgentDiary** 定名，agent-journal 列别名 | 4✅ 确认制走完 |

票态账：**若若五票全回（183 号信）**；Cora/Veda/Eve 截至 23:00 未回票=按缺席记录不追（Eve 注入攻防题同规则，其 09-16 五题答卷已在输入内）；Alice 的 301/310/313 三信视为方案票（V1/V2/V3 的案主与实证来源）。§7 嵌入索引阈值若若注：复用 v1.2.0 P2 内容指纹缓存，零新增成本。

## 10. 里程碑

09-17 草案（RC1）→ 09-17 23:00 收票（若若全回，三家缺席记录）→ **RC2（本文件，票果已归纳）** → 09-20 外沿 **1.0 定稿** → MVP 立项**等爸爸拍板**后由若若动代码（范围=V1 Alice 案：第一周情景日志+rolling handoff）、验收官跑 §8 全单。
